  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- Projects -->
        <div
          class="p-6 bg-gradient-to-br from-tech-blue/10 to-transparent rounded-2xl border border-white/10"
        >
          <div class="flex items-center justify-between mb-4">
            <span class="text-3xl">📁</span>
            <span class="text-xs text-tech-blue bg-tech-blue/10 px-2 py-1 rounded-full">{{
              'DASH_CARD_NEW_MONTH' | translate
            }}</span>
          </div>
        ...
        <!-- Contacts -->
        <div
          class="p-6 bg-gradient-to-br from-green-500/10 to-transparent rounded-2xl border border-white/10"
        >
          <div class="flex items-center justify-between mb-4">
            <span class="text-3xl">📬</span>
            <span class="text-xs text-green-400 bg-green-500/10 px-2 py-1 rounded-full"
              >{{ unreadContacts() }} {{ 'DASH_CARD_NEW_MSG' | translate }}</span
            >
          </div>
        ...
        <!-- Translations -->
        <div
          class="p-6 bg-gradient-to-br from-purple-500/10 to-transparent rounded-2xl border border-white/10"
        >
          <div class="flex items-center justify-between mb-4">
            <span class="text-3xl">🌍</span>
            <span class="text-xs text-purple-400 bg-purple-500/10 px-2 py-1 rounded-full">{{
              'DASH_CARD_LANGUAGES' | translate
            }}</span>
          </div>
        ...
        <!-- Technologies -->
        <div
          class="p-6 bg-gradient-to-br from-orange-500/10 to-transparent rounded-2xl border border-white/10"
        >
          <div class="flex items-center justify-between mb-4">
            <span class="text-3xl">⚡</span>
            <span class="text-xs text-orange-400 bg-orange-500/10 px-2 py-1 rounded-full"
              >Stack</span
            >
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white/5 rounded-2xl border border-white/10 p-6">
          <div class="flex items-center justify-between mb-6">
            ...
            <button class="text-tech-blue text-sm hover:underline">
              {{ 'DASH_BTN_VIEW_ALL' | translate }}
            </button>
          </div>
      ...
      <div class="bg-white/5 rounded-2xl border border-white/10 p-6">
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex gap-2">
            <button
              ...
              [class]="
                period() === 7 ? 'bg-tech-blue/10 text-tech-blue' : 'text-gray-400 hover:bg-white/5'
              "
        ...
        <div class="h-64 flex items-end gap-2 px-4">
          @for (height of chartData(); track $index) {
            <div
              class="flex-1 bg-gradient-to-t from-tech-blue/50 to-tech-blue rounded-t-lg transition-all duration-500 hover:from-tech-blue/70"
              [style.height.%]="height"
            ></div>
          }
        </div>
  ...
  activities = signal<Activity[]>([
    {
      id: '1',
      type: 'contact',
      messageKey: 'DASH_ACT_CONTACT_MSG',
      timeKey: 'ACT_TIME_HOURS',
      timeParam: '2',
    },
    {
      id: '2',
      type: 'project',
      messageKey: 'DASH_ACT_PROJECT_MSG',
      timeKey: 'ACT_TIME_HOURS',
      timeParam: '5',
    },
    {
      id: '3',
      type: 'translation',
      messageKey: 'DASH_ACT_TRANS_MSG',
      timeKey: 'ACT_TIME_YESTERDAY',
      timeParam: '1',
    },
  ]);
