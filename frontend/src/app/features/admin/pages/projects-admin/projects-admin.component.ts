  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
        <table class="w-full">
          <thead class="bg-white/5">
            <tr>
              <th class="text-left px-6 py-4 text-gray-400 font-medium text-sm">
                {{ 'ADMIN_TBL_NAME' | translate }}
              </th>
              <th class="text-left px-6 py-4 text-gray-400 font-medium text-sm">
                {{ 'ADMIN_TBL_TECHNOLOGIES' | translate }}
              </th>
              <th class="text-left px-6 py-4 text-gray-400 font-medium text-sm">
                {{ 'ADMIN_TBL_STATUS' | translate }}
              </th>
              <th class="text-left px-6 py-4 text-gray-400 font-medium text-sm">
                {{ 'ADMIN_TBL_DATE' | translate }}
              </th>
              @if (adminAuth.canEdit()) {
                <th class="text-right px-6 py-4 text-gray-400 font-medium text-sm">
                  {{ 'ADMIN_TBL_ACTIONS' | translate }}
                </th>
              }
            </tr>
          </thead>
          <tbody>
            @for (project of projects(); track project.id) {
                ...
                <td class="px-6 py-4">
                  <div class="flex flex-wrap gap-1">
                    @for (tech of project.technologies.slice(0, 3); track tech.name) {
                      <span
                        class="px-2 py-0.5 bg-tech-blue/10 text-tech-blue text-xs rounded-full"
                        >{{ tech.name }}</span
                      >
                    }
                    @if (project.technologies.length > 3) {
                      <span class="px-2 py-0.5 bg-white/5 text-gray-400 text-xs rounded-full"
                ...
                <td class="px-6 py-4">
                  @if (project.isActive) {
                    <span class="px-3 py-1 bg-green-500/10 text-green-400 text-xs rounded-full">{{
                      'ADMIN_STATUS_ACTIVE' | translate
                    }}</span>
                  } @else {
                    <span class="px-3 py-1 bg-gray-500/10 text-gray-400 text-xs rounded-full">{{
                      'ADMIN_STATUS_INACTIVE' | translate
                    }}</span>
                  }
                </td>
              ...
              <tr>
                <td
                  [attr.colspan]="adminAuth.canEdit() ? 5 : 4"
      ...
      @if (showModal && adminAuth.canEdit()) {
        ...
        >
          <div
            class="bg-graphite-900 rounded-2xl border border-white/10 w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-white/10"
          >
            <h2 class="text-xl font-bold text-white mb-6">
              {{
                (editingProject ? 'ADMIN_PROJECTS_MODAL_EDIT' : 'ADMIN_PROJECTS_MODAL_NEW')
                  | translate
              }}
            </h2>
            ...
            <form (submit)="saveProject($event)" class="space-y-4">
              ...
              <div>
                <label class="block text-gray-400 text-sm mb-2">{{
                  'ADMIN_FORM_DESCRIPTION' | translate
                }}</label>
                <textarea
                  [(ngModel)]="form.description"
                  name="description"
                  rows="3"
                  class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-tech-blue/50 resize-none"
                ></textarea>
              </div>

              <div>
                <label class="block text-gray-400 text-sm mb-2">{{
                  'ADMIN_FORM_TECH_HINT' | translate
                }}</label>
                <input
                  type="text"
                  [(ngModel)]="form.technologiesStr"
                  name="technologies"
                  class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-tech-blue/50"
              ...
              <div class="flex items-center gap-3">
                ...
                />
                <label for="isActive" class="text-gray-300">{{
                  'ADMIN_FORM_PROJECT_ACTIVE' | translate
                }}</label>
              </div>
...
export class ProjectsAdminComponent implements OnInit {
  ...
  saveProject(event: Event) {
    ...
    const techNames = this.form.technologiesStr
      .split(',')
      .map((t) => t.trim())
      .filter((t) => t);
    const technologyIds = techNames
      .map((name) => {
        const tech = this.availableTechnologies().find(
